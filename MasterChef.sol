...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accEggPerShare = accEggPerShare.add(eggReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        /*
        Example to calculate eggReward

        With 1 egg per block
        ...
        1 * 3(multiplier) * 2500 = (7500) / 10000(totalAllocPoint) = 0.75 Egg tokens to be minted
        */
        uint256 eggReward = multiplier.mul(EggPerBlock).mul(pool.allocPoint).div(totalAllocPoint); 
        ...
        pool.accEggPerShare = pool.accEggPerShare.add(eggReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
